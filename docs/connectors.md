...
Task 3.4 added durable watermark state: the `connector_poll_state` table (`connector_name` PK, `last_polled_at`). The poll orchestrator (`lib/connectors/poll.ts`) resolves each connector's `since` as **explicit override > stored `last_polled_at` > `now − 24h`**, and advances the watermark to the **poll-start** instant — only for connectors that polled fully clean. A connector that threw, or had ANY per-payload ingest failure, keeps its old watermark so the next poll retries that window (at-least-once, made safe by `evidence.dedupe_key`). The advance is a single atomic `ON CONFLICT DO UPDATE SET last_polled_at = max(stored, excluded)` — lexical `max` over fixed-width ISO-8601-`Z` is chronological, so two overlapping polls can't move the watermark backwards. A per-connector **config-hash** column (invalidate the watermark when a connector's watch list / fixture changes, mirroring `routing_assignments.routing_rules_hash`) is a documented future refinement, deliberately not built ("even if simple" bounded Task 3.4).

## Poll orchestration (endpoint + scheduler)

`POST /api/connectors/poll` and `scripts/poll-connectors.ts` are **thin wrappers over one shared orchestrator** — `pollConnectors()` + `recomputeAffectedAccounts()` in `lib/connectors/poll.ts`. No orchestration logic is duplicated between the two entry points.

- **Per-connector isolation is absolute.** One connector throwing (including GitHub's documented all-or-nothing `ConnectorError`) becomes that connector's `{ ok:false, error }` in the response; every other connector still polls and ingests. A failing connector is never a swallowed warning and never blocks the others. The endpoint returns HTTP 200 even on partial failure — a reported failure is data, not a transport error.
- **The response does not overstate.** A connector's `ok` is `fetch succeeded AND zero per-payload ingest failures`; the top-level `ok` is the AND of every connector and every recompute. A recompute-config problem reports `ok:false` but never hides or rolls back the evidence that did ingest.
- **`recomputeAffectedAccounts` is the single enforcement point** for the config-before-mutation invariant: `routing-rules.md` AND `DEFAULT_OWNER_EMAIL` are validated up front; a bad value fails every affected account with **zero `computeScore` calls** (no dangling `lead_scores` rows). The endpoint and scheduler do NOT re-implement this check — a caller-duplicated invariant is itself a drift bug.
- **Deferred seam (now-feasible follow-up):** `recomputeAffectedAccounts` deliberately mirrors `/api/scoring/recompute`'s gating (tier_promotion gated on `score.inserted`; engagement_spike always; alert failures never fail recompute; config-before-mutation) rather than sharing one core. The route IS covered by `tests/integration/inbound-pipeline.test.ts`, so a unification into a single `recomputeAccount` core is feasible with a regression net — it's deferred purely as a Task-3.4 scope boundary (refactoring the route's hardened HTTP handler is a separate change), not a capability gap. Parity is pinned by tests on both sides.
