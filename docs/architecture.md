...
The architecture is designed for **one Node process** talking to a local SQLite file. A few correctness arguments rely on that, but in different ways depending on the layer — and the differences matter when you try to scale out.

1. **Scoring recompute idempotency is process-local.** `computeScore` in `lib/scoring/score.ts` is *not* wrapped in `db.transaction`. It does a latest-row SELECT (ordered by `rowid DESC`), compares the fresh fingerprint to that row, and either short-circuits or inserts. The earlier draft had a UNIQUE `(account_id, fingerprint)` index as a defense for that SELECT-then-INSERT gap, but the same index blocked legitimate state recurrence (cold → warm → cold returning to cold should write a new row, not collide with the original cold row). It was dropped in `db/migrations/0006_daffy_lionheart.sql`, leaving a non-unique index for query speed only. What keeps the SELECT-then-INSERT pair correct *today* is that `better-sqlite3` is **synchronous** and `computeScore` has no `await` between the latest-row SELECT and the INSERT — two recomputes in this Node process literally cannot interleave. Two processes can.
2. **Signal ingest idempotency is enforced at the DB layer.** `ingestSignal` in `lib/signals/ingest.ts` *is* wrapped in `db.transaction`, and `evidence.dedupe_key` is `UNIQUE` in `db/schema.ts`. The insert path catches unique violations and re-selects the winner by `dedupe_key`. The *pattern* is portable across processes (the unique index is enforced by SQLite globally, not per-transaction) and across database engines, but the *current code* is SQLite-specific in one place: `isUniqueViolation` in `lib/signals/ingest.ts` only matches `SQLITE_CONSTRAINT_UNIQUE` / `SQLITE_CONSTRAINT_PRIMARYKEY`. Porting to Postgres requires also recognizing `23505`; otherwise the catch path rethrows under contention instead of re-selecting.
3. **Scoring's latest-row ordering uses `rowid DESC`.** The "most recent score for this account" SELECT depends on SQLite's monotonic per-table `rowid`. Ingest doesn't use it — ingest resolves by `dedupe_key`, not insert order. `rowid` is a SQLite-only primitive; it doesn't exist on Postgres.

**What breaks where:**

- **Multiple Node processes pointing at the same SQLite file.** *Scoring* breaks: two recomputes for the same account can both run their latest-row SELECT, both see the same prior row, and both insert. There is no unique constraint left to reject either. *Ingest does not break here* — the unique `dedupe_key` index still rejects the duplicate and forces both writers down the catch-and-reselect path.
- **Postgres or any networked database.** *Ingest mostly works* under its existing pattern, but the SQLite-specific error code check in `isUniqueViolation` needs to be widened to also accept `23505` before the catch-and-reselect path triggers. *Scoring's `rowid DESC` ordering does not work*; you'd need a real monotonic column (the comment at `lib/scoring/score.ts:162` explicitly rules out `(computedAt, id)` because timestamps tie and ids have a random hex suffix), and the SELECT-then-INSERT race remains regardless of the engine.
- **Serverless / Edge runtimes.** Each invocation is a fresh process. Two near-simultaneous requests are two independent processes by definition, so the scoring race surfaces immediately even on SQLite.

**The concrete fix for scoring**, in any of those settings, is to make concurrent writers either serialize on the database or self-reject duplicates *and* recover when they lose:

- Either add a `sequence INTEGER NOT NULL` column per row, `UNIQUE (account_id, sequence)`, compute `next = max(sequence) + 1` inside a transaction, AND wrap the insert in a catch-on-unique-violation that recomputes `next` and retries. The unique constraint alone won't make both writers succeed — the loser must retry.
- Or include a chain indicator inside the fingerprint that names the previous row's id (so state recurrence under a new prior produces a new fingerprint), paired with a re-introduced `UNIQUE (account_id, fingerprint)` index AND the same catch-on-unique-violation path so the loser re-runs against the freshly-committed prior. **The chain indicator alone is not sufficient** — without enforced uniqueness, two writers seeing the same prior row still produce identical chained fingerprints and both still insert.

This isn't a bug to defer — it's a *deployment constraint*. The v2 design is correct for its target. If the operator moves it beyond that target, the scoring concurrency layer needs to be redesigned before the move, not after the first race surfaces.
