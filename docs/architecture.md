...
2. **Signal ingest idempotency is enforced at the DB layer.** `ingestSignal` in `lib/signals/ingest.ts` *is* wrapped in `db.transaction`, and `evidence.dedupe_key` is `UNIQUE` in `db/schema.ts`. The insert path catches unique violations and re-selects the winner by `dedupe_key`. That story is portable across processes — the unique index is enforced by SQLite globally, not per-transaction — and across database engines.
3. **Latest-row ordering uses `rowid DESC`.** Scoring's "most recent score for this account" SELECT depends on SQLite's monotonic per-table `rowid`. Ingest doesn't use it — ingest resolves by `dedupe_key`, not insert order. `rowid` is a SQLite-only primitive; it doesn't exist on Postgres.
...
- **Postgres or any networked database.** *Ingest still works* under its existing mechanism (catch-on-unique-violation is portable). *Scoring's `rowid DESC` ordering does not work*; you'd need an explicit monotonic column (a `sequence` per account, or `created_at` plus a deterministic tiebreaker) to recover "most recent row for this account." And the SELECT-then-INSERT race remains regardless of the engine.
...
**The concrete fix for scoring**, in any of those settings, is to make concurrent writers either serialize on the database or self-reject duplicates. Either a `sequence` column per account with `UNIQUE (account_id, sequence)` (and `next_sequence = max + 1` computed inside a transaction), or a chain indicator inside the fingerprint that includes the *previous* row's id, paired with a re-introduced `UNIQUE (account_id, fingerprint)` index. **The chain indicator alone is not sufficient** — without an enforced uniqueness, two writers seeing the same prior row still produce identical chained fingerprints and both still insert.
